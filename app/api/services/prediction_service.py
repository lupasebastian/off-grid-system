...
from app.utils.global_utils import extract_weather_hourly_variables_by_name
import config
...
class SolarPredictionService:
    ...
    async def get_pvlib_prediction(self, start, end):
        filename = f'generation_profiles/{start} - {end} {config.LONGITUDE} {config.LATITUDE}_{config.NUMBER_OF_PANELS}_panels.csv'.replace(
            ' ', '_').replace(':', '-')
        # try:
