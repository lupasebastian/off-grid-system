...
from utils import generate_demand, generate_digester_thermal_load
from pv_prediction import get_pvlib_prediction
...
sim_start = datetime.datetime.combine(datetime.datetime.now().replace(day=1, month=1, year=config.SIMULATION_YEAR), datetime.time.min)
sim_end = datetime.datetime.combine(sim_start.replace(month=12, day=31), datetime.time.max).replace(microsecond=0)
n.set_snapshots(pd.date_range(start=sim_start, end=sim_end, freq='h'))
...
solar_profile_raw = get_pvlib_prediction(start=sim_start, end=sim_end)
...
chp_max_profile = pd.Series(1.0, index=n.snapshots)
night_max_hours = chp_max_profile.index.hour < 5
chp_max_profile[night_max_hours] = 0.0

chp_min_profile = pd.Series(0.85, index=n.snapshots)
...
n.add("Link", "CHP_Biogas_Generator",
      ...
      bus1="Electricity_Bus",
      bus2="Heat_Bus",
      efficiency=config.ELECTRICAL_EFFICIENCY, # efficiencies based on an example of turning 120 kW of fuel into 50 kW el and 60 kW of heat
...
chps_conversion_losses_mwh = abs(biogas_cons_chp1_mwh) + abs(biogas_cons_chp2_mwh)  - (abs(chp_mwh) + abs(chp2_mwh) + (abs(biogas_cons_chp1_mwh + biogas_cons_chp2_mwh) * config.CHP_THERMAL_EFFICIENCY))
...
absolute_total_energy_error_mwh = abs(total_system_inputs_mwh - (total_system_outputs_mwh + conversion_losses_mwh))
